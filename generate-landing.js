...
// Facebook Pixel
let facebookCode = '';
if (CONFIG.analytics.facebookPixel.enabled) {
  facebookCode = `<script>
...
fbq('track', 'PageView');
  ...
  <noscript><img height="1" width="1" style="display:none" src="https://www.facebook.com/tr?id=${CONFIG.analytics.facebookPixel.pixelId}&ev=PageView&noscript=1"/></noscript>`;
}
html = html.replace(/{{ANALYTICS_FACEBOOK}}/g, facebookCode);

// Google Analytics
let googleCode = '';
if (CONFIG.analytics.googleAnalytics.enabled) {
  googleCode = `<script async src="https://www.googletagmanager.com/gtag/js?id=${CONFIG.analytics.googleAnalytics.measurementId}"></script>
  <script>
...
html = html.replace(/{{ANALYTICS_GOOGLE}}/g, googleCode);

// Яндекс.Метрика
let yandexCode = '';
if (CONFIG.analytics.yandexMetrika.enabled) {
  yandexCode = `<script type="text/javascript">
    ...
    ym(${CONFIG.analytics.yandexMetrika.counterId}, "init", {
      clickmap:true,
      trackLinks:true,
  ...
  <noscript><div><img src="https://mc.yandex.ru/watch/${CONFIG.analytics.yandexMetrika.counterId}" style="position:absolute; left:-9999px;" alt="" /></div></noscript>`;
}
html = html.replace(/{{ANALYTICS_YANDEX}}/g, yandexCode);
